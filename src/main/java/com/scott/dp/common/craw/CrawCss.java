 ...
 * @Status : 编写
 **/
public class CrawCss {
    private static Log log = LogFactory.getLog(CrawCss.class);
    // 获取img标签正则
    private static final String cssURL_REG = "<link.*href=(.*?)[^>]*?>";
    // 获取src路径的正则
    private static final String cssSRC_REG = "[a-zA-z]+://[^\\s]*";
    private static List<String> downList = new ArrayList<String>();

    public static List<String> carwCss(String url,String savePath) {
        try {
            //获得html文本内容
            String HTML = getHtml(url);
            List<String> imgUrl = getcssUrl(HTML);
            List<String> cssUrl = new ArrayList<String>();
            for (String css:imgUrl) {
                css = css.split("href=\"")[1].split(".css")[0]+".css";
                cssUrl.add(css);
            }
            //下载
            download(cssUrl,savePath);
    ...
    private static List<String> getcssUrl(String html){
        Matcher matcher=Pattern.compile(cssURL_REG).matcher(html);
...
//                下载css
                try {
                    downLoadFromUrl(url, savePth + "css/");
